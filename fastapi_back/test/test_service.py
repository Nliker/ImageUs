...
@pytest.fixture
def user_service():
    return UserService(UserDao(database),settings)

@pytest.fixture
def room_service():
    return RoomService(RoomDao(database),settings)

@pytest.fixture
def image_service():
    return ImageService(ImageDao(database),settings)
...
def test_upload_image(image_service):
    ...
    parent_path=os.path.dirname(os.path.abspath(os.path.dirname(__file__)))
    test_image_path=f"{parent_path}/{settings.TEST_IMAGE_PATH}/{filename}"
    ...
    must_new_image_link=f"{settings.IMAGE_DOWNLOAD_URL}1/{filename}"
...
def test_upload_room_image(image_service):
    ...
    new_image_id=result['new_image_id']
    new_image_link=f"{settings.IMAGE_DOWNLOAD_URL}1/{filename}"
    new_image_info=image_service.get_image_info(new_image_id)
    assert new_image_info=={
        'id':5,
        'link':new_image_link,

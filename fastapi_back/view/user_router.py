...
@user_router.post("/sign-up",status_code=200,
...
async def post_sign_up(req: post_sign_up_request):
        ...
        if await user_router.services.user_service.is_email_exists(new_user['email']):
            error.email_existance_sign_up_error().get_raise()
            
        new_user_id=await user_router.services.user_service.create_new_user(new_user)
        user_info=await user_router.services.user_service.get_user_info(new_user_id)
...
async def post_login(req:post_login_request):
    ...
    if not user_router.services.user_service.is_email_exists(credential['email']):
